...
void solve(int CASE) {
    int n;
    cin >> n;
    MCMF mcmf(n*3+1, 0, n*3+1);
    vector<int> a(n+1), h(n+1), l(n+1);
    for (int i=1; i<=n; i++) {
        cin >> a[i];
    }
    int num = max_element(a.begin(), a.end()) - a.begin();
    for (int i=1; i<=n; i++) {
        cin >> h[i];
    }
    for (int i=1; i<=n; i++) {
    ...
    for (int i=1; i<=n; i++) {
        for (int j=1; j<=n; j++) {
            if (a[i] <= a[j]) continue;
            mcmf.addEdge(i+n*2, j, ((i == num) ? (l[i]) : (l[i]-1)), -(a[i]^a[j])+h[i]+h[j]);
        }
    }
    for (int i=1; i<=n; i++) {
        mcmf.addEdge(i, i+n, 1, 0);
        mcmf.addEdge(mcmf.s, i+n*2, mcmf.inf, 0);
