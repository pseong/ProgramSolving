    ...
    for (int i=0; i<n; i++) {
        for (int j=0; j<n; j++) {
            ...
            for (int d=0; d<2; d++) {
                ...
                int y = conv(a, b);
                mcmf.addEdge(x+bias, y, mcmf.inf, 0);
            }
            int w;
            cin >> w;
            mcmf.addEdge(x, x+bias, 1, -w);
            mcmf.addEdge(x, x+bias, mcmf.inf, 0);
        }
    }
    mcmf.addEdge(mcmf.s, 1, k, 0);
    mcmf.addEdge(n*n+bias, mcmf.t, k, 0);
